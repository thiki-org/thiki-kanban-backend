...
public class BoardResource extends RestResource {
    ...
    public Object toResource(String projectId, String userName) throws Exception {
    ...
    public Object toResource(Board board, String projectId, String userName) throws Exception {
        logger.info("build board resource.board:{},userName:{}", board, userName);
        BoardResource boardResource = new BoardResource();
        boardResource.domainObject = board;
        this.domainObject = board;
        if (board != null) {
            Link selfLink = linkTo(methodOn(BoardsController.class).findById(board.getId(), projectId, userName)).withSelfRel();
            ...
            Link activeSprintLink = linkTo(methodOn(SprintController.class).loadActiveSprint(board.getId(), projectId, userName)).withRel("activeSprint");
            boardResource.add(tlink.from(activeSprintLink).build(userName));

            Link pagesLink = linkTo(methodOn(PagesController.class).findByBoard(board.getId(), projectId, userName)).withRel("pages");
